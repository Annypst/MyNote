...
- StringBuffer类：字符串缓冲区，他的长度和内容都是可变的，而原本字符串，一旦创建，都不可变了。StringBuffer像一个字符容器，修改时，并不会产生新的对象。

  <img src="assets/image-20260106131854962.jpg" alt="image-20260106131854962" style="zoom:33%;" />

  

- StringBuilder：与上面Buffer一样，可以修改，但是是线程不安全的，所以意味着它可以被同步访问。
...
> [!WARNING]
>
> equls()方法并没有覆盖StringBuilder、StringBuffer，仍然比较的是地址，所以两个不同对象，即使内容一样，结构是false。
...
<img src="assets/IMG_9458.jpg" alt="IMG_9458" style="zoom: 25%;" />

<img src="assets/IMG_9459.jpg" alt="IMG_9459" style="zoom: 25%;" />

```java
package com.annypst.gui;

import javax.swing.*;

public class jframe extends JFrame {
    private static void creatAndShowGUI(){
        JFrame j1=new JFrame("Test");//creating a new frame
        j1.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);//setting the default close operation to exit on close
        j1.setTitle("tttteeesssttt");//setting the title of the frame
        j1.setSize(200,300);
        j1.setVisible(true);
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(jframe::creatAndShowGUI);//calling the method to create and show the GUI
    }
}
...
![IMG_9460](assets/IMG_9460.jpg)

setModal()方法

- 布局管理器一共有四种：FlowLayout流式  Border边界（JFream默认）  GridLayout网格   GridBagLayout 网格包
...
> 主要用于响应用户操作：
>
> - 事件源 EventSource：事件发生的场所，按钮、窗口、菜单
>
>   事件对象Event：发生的操作，比如用户的特定操作
>
> - 监听器Listener：监听事件，做出处理（也有事件处理器）
...
- 字节流：字节输入流、字节输出流，提供两个抽象类：InputStream OutputStream
- 字节缓冲流：BufferedInputStream  BufferedOutputStream
- 字符流： FileReader read()方法、FileWriter write方法
- 转换流：完成字节流和字符流之间的转换 将字节流转换为字符流 InputStreamReader  InputStreamWriter
...
<img src="assets/image-20260106120517571.jpg" alt="image-20260106120517571" style="zoom:33%;" />
...
这是 Java 面向对象的核心特性（也叫运行时多态），核心逻辑是：**父类引用指向子类对象时，调用重写的方法，实际执行的是子类的实现，而非父类的实现**，动态多态（运行时多态）：当你把**子类对象赋值给父类类型的变量**（即「向上转型」），调用该变量的**重写方法**时，JVM 不会根据「变量的声明类型（父类）」执行，而是根据「变量指向的实际对象类型（子类）」执行，这个决策过程发生在**程序运行时**，而非编译时。
...
public class PolymorphismDemo {
    public static void main(String[] args) {
        // 向上转型：父类引用（Animal）指向子类对象（Dog）
        Animal animal = new Dog();
        
        // 调用重写的eat方法：实际执行子类Dog的eat（动态多态核心体现）
        animal.eat(); // 输出：狗啃骨头（而非“动物进食”）
        
        // 注意：父类引用无法调用子类独有方法（编译报错）
...
子类必须**重写**父类的方法（如果父类方法是`static/final/private`，无法重写，也就无多态）；

必须发生「向上转型」（父类引用指向子类对象）。
...
> [!NOTE]
>
> 将运行的代码放入public void run()中 

为了实现线程的安全性，可使用关键字synchronized（Obj lock）来同步线程，要注意对异常的处理。
...
`ArrayList<T>`作为类的成员变量（实例变量），核心作用是让类的每个对象持有一个动态数组集合，用于存储一组同类型的数据（比如一个`Student`类持有学生的课程列表、一个`Shop`类持有商品列表）。



## ArrayList 的遍历方法

ArrayList 的遍历是开发中最常用的操作，核心有 4 种方法，适用于不同场景，下面按「常用程度 + 适用场景」排序讲解：

### 1. 增强 for 循环（foreach）：最简洁，推荐优先使用

- **适用场景**：仅需遍历元素，无需操作索引（最常用）。
- **语法**：`for (元素类型 变量名 : 集合名) { 操作元素 }`
- **示例**：
...
public class ArrayListTraverse {
    public static void main(String[] args) {
        ...
        for (String language : list) {
            System.out.println("元素：" + language);
        }
    }
}
```
...
### 2. 普通 for 循环：需要操作索引时使用

- **适用场景**：遍历过程中需要使用索引（比如修改指定位置元素、跳过 / 重复处理某个索引）。
- **语法**：`for (int i = 0; i < 集合.size(); i++) { 操作list.get(i) }`
- **示例**：
...
for (int i = 0; i < list.size(); i++) {
    ...
    if (i == 1) {
        list.set(i, element + "（修改后）"); // 修改索引1的元素
    }
...
### 3. 迭代器（Iterator）：需要遍历中删除元素时使用

- **适用场景**：遍历过程中需要删除元素（避免`ConcurrentModificationException`并发修改异常）。
- **核心方法**：`hasNext()`（判断是否有下一个元素）、`next()`（获取下一个元素）、`remove()`（删除当前元素）。
- **示例**：
...
System.out.println("删除后集合：" + list); // 输出：[Python（修改后）, C++]
```

### 4. Lambda 表达式（Java 8+）：简洁的函数式遍历

- **适用场景**：Java 8 及以上，追求代码简洁，适合简单的遍历操作。
- **语法**：`集合.forEach(元素 -> 操作元素)`
- **示例**：
...
```
// Lambda表达式遍历
list.forEach(element -> System.out.println("Lambda遍历：" + element));
```
...
- **增强 for 循环 / 迭代器遍历中不能直接修改集合**：比如用`list.remove(element)`会抛出`ConcurrentModificationException`，必须用迭代器的`remove()`方法。
- **`list.get(i)`的效率**：ArrayList 是基于数组实现的，`get(i)`是 O (1) 时间复杂度，遍历效率高（对比 LinkedList 的`get(i)`是 O (n)）。
- **空集合处理**：遍历前建议判断`list != null && !list.isEmpty()`，避免空指针或遍历空集合。
